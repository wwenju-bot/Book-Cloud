<template>
  <el-dialog :title="`「${noticeTitle}」已读用户`" :visible.sync="visible" width="760px" top="6vh" append-to-body @close="handleClose">
    <el-form :model="querailParams" ref="queryForm" size="small" :inline="true" style="margin-bottom: 4px;">
      <el-form-item prop="searchValue">
        <el-input
          v-model="querailParams.searchValue"
    ...
    <pagination v-show="total > 0" :total="total" :page.sync="querailParams.pageNum" :limit.sync="querailParams.pageSize" @pagination="getList" style="padding: 6px 0px;"/>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    open(row) {
      ...
      this.querailParams.noticeId = row.noticeId
      this.querailParams.searchValue = undefined
    ...
    getList() {
      this.loading = true
      listNoticeReadUsers(this.querailParams).then(res => {
